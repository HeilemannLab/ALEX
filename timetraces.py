...
def binnedTrace1(array):
    ...
    for i in range(len(trace)):
        bins = 0
        t_m = (i + 1) * 1e6
        while (array[j] < t_m) and (array[j] >= t_m - (1e6)):
            bins += 1
            j += 1
        trace[i] = bins
    return trace, n_bin
...
green_trace, num1 = binnedTrace1(green_trace[:, 0])
green_trace[:] = [x / 10.0 for x in green_trace]
num1 = np.arange(num1)

red_trace, num2 = binnedTrace1(red_trace[:, 0])
