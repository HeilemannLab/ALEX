...
class TimestampingCounters:
    ...
    def startCounting(self):
        """
        Four counters are set up, all counting edges, source terminal of counter 0 and 2 is 100MHzTimebase,
    ...
    def SamplingCounter1(self):
        """
        Sampling of all four counters is set to sample clock, all are sampled by the signal of their APD respectively.
        The signal should be received by the source terminal. SetSampClkOverrunBehavior sets the task to ignore sample pulses,
        that came faster than the task can handle. This might be the case when a fluorophore enters the volume,
        it also means we lose all photon events that are coming in faster than 100MHz.
        The terminal count signal of counters 0 and 2 is routed to the output terminals of counters 1 and 3 respectively.
        Output behavior is 'puls' (not toggle). This is important because only rising edges get counted.
        DuplicationPrevention is recommended by NI for counter tasks, where the sampling is slower that the input.
        """
    ...
    def ArmStartCounter1(self):
        """
        Arm start trigger makes the counters wait for an external signal. Signal could be the timing task from 6713 (over RTSI).
        But counters should always be started first, otherwise they wait for the next edge or infinitely.
        """
    ...
    def readAPD(self):
        """The data is written from the arrays into a single data array which gets returned."""
        if self._N == 1:
    ...
    def actualCounting(self, task, readArray):
        """
        Read the data from the cards queue into 'readArray'. Timeout is the time to wait until the last samples are written.
        Returns the readArray
