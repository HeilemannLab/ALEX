...
class DigitalSignal:
    ...
    def InitIllumination(self):
        """
        The counter output tasks are configured as PulseChanTime, which is fairly flexible.
        The tasks are triggered by an ArmStartTrigger, which makes it wait for the next trigger
        edge. Also the analog signal gets started here. The semaphore indicates, that initiation
        is done.
        """
        self.calcSignal()
